...
mpi_func_id_prefix = "$crate::qmpi_sys::_MPI_funcs::_MPI_"

for qmpi_type in qmpi_types:
    type_map[qmpi_type] = "$crate::qmpi_sys::%s" % qmpi_type
for mpi_type in mpi_types:
    type_map[mpi_type] = "$crate::mpi_sys::%s" % mpi_type

for c_type, rust_type in type_map.items():
    type_replacements.update({
        (c_type, "") : rust_type,
