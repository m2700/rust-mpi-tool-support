...
impl<'c> Process<'c> {
    ...
    #[inline]
    pub fn send<B: Buffer + ?Sized>(&self, buffer: &B, tag: Tag) -> RmpiResult {
        unsafe {
            self.send_with(
    ...
    #[inline]
    pub fn bsend<B: Buffer + ?Sized>(&self, buffer: &B, tag: Tag) -> RmpiResult {
        unsafe {
            self.bsend_with(
                |buf, count, datatype, rank, tag, comm| {
    ...
    #[inline]
    pub fn ssend<B: Buffer + ?Sized>(&self, buffer: &B, tag: Tag) -> RmpiResult {
        unsafe {
            self.ssend_with(
                |buf, count, datatype, rank, tag, comm| {
    ...
    #[inline]
    pub fn rsend<B: Buffer + ?Sized>(&self, buffer: &B, tag: Tag) -> RmpiResult {
        unsafe {
            self.rsend_with(
                |buf, count, datatype, rank, tag, comm| {
