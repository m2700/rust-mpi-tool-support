...
impl Request {
    ...
    tool_mode_item! {
        ...
        pub unsafe fn free_with<F>(mut self, mpi_free: F) -> RmpiResult
            where F: FnOnce(*mut MPI_Request) -> c_int
        {
...
impl RequestSlice {
    ...
    tool_mode_item! {
        ...
        pub unsafe fn waitany_with<F>(&mut self, mpi_waitany: F) -> RmpiResult<(usize, Status)>
            where F: FnOnce(c_int, *mut MPI_Request, *mut c_int, *mut MPI_Status) -> c_int
        {
            let mut indx = 0;
            let mut status = MaybeUninit::uninit();
            Error::from_mpi_res(
                mpi_waitany(self.0.len() as c_int, self.0.as_mut_ptr(), &mut indx, status.as_mut_ptr())
            ).map(|()|{
                (indx as usize, Status::from_raw(status.assume_init()))
            })
        }
    }
    #[inline]
